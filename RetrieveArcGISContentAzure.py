...
#todo: get curretn AT Eomployees
def getATEmployees(employeeFile):
    fs = open(employeeFile)
    emailLst = []
    userLst = []
    for f in fs:
        lstTable = f.split(",")
        userLst.append(string.strip(lstTable[3]) + "@TRANSPORT")
        emailLst.append(lstTable[2])
    fs.close()
    return emailLst
...
def mapPortalUsers(allUserList):
    ...
    adminUserList = ['svc_ArcGis@TRANSPORT', 'SusanJon1@TRANSPORT']
...
def generateRoles(folder):
    ...
    parameters = urllib.urlencode({ 'f' : 'json', 'token': token}) #remember to generate json content
    response = urllib2.urlopen(url, parameters ).read()
    jsonResponse = json.loads(response)
    id = jsonResponse['id']

    #get Role url
    url = portalUrl + 'sharing/rest/portals/' + id + '/roles'
    setSecurity (url, u, p)
    
    response = urllib2.urlopen(url, parameters ).read()
    jsonResponse = json.loads(response)

    #todo: get The Custom Roles
    print("\nFetch the custom Roles")
    ...
    fs = open(folder + os.path.sep + "PortalRoles.csv", "w+")
    fs.write("RoleId,RoleName,Description,Privilege\n")

    #todo: get The Permissions For Each Role
    for role in roles:
        roleId = role['id']
        roleUrl = portalUrl + 'sharing/rest/portals/' + id + '/roles/' + roleId + '/privileges'
        setSecurity (url, u, p)
        parameters = urllib.urlencode({ 'f' : 'json', 'token': token}) #remember to generate json content
        roleResponse = urllib2.urlopen(roleUrl, parameters ).read()
        jsonRolePrivileges = json.loads(roleResponse)
        privileges = jsonRolePrivileges['privileges']
        #print(jsonRolePrivileges)
        for pr in privileges:
            fs.write( roleId + ',' + role['name'] + ',' + role['description'] + ',' + pr + '\n') #username,fullName,created      
    fs.close()
...
def generateUsers(folder):
    ...
    #todo: get Total Content
    parameters = urllib.urlencode({ 'f' : 'json', 'token': token, 'start': start, 'num': num}) #remember to generate json content
    response = urllib2.urlopen(url, parameters ).read()
    jsonResponse = json.loads(response)
    id = jsonResponse['id']
    print(id)
    ...
    #get Role url
    url = portalUrl + 'sharing/rest/portals/' + id + '/users'
    print(url)
    setSecurity (url, username, password)
    ...
    fsi = open(folder + os.path.sep + "PortalContent.csv", mode = "w+")
    fs.write("UserName,Description,Access,Created,Modified\n")
    fsi.write("Owner,Title,URL,NumViews,Type\n")
    while f < total + 1: 
        parameters = urllib.urlencode({ 'f' : 'json', 'token': token, 'start': start, 'num': num}) #remember to generate json content
        response = urllib2.urlopen(url, parameters ).read()
        jsonResponse = json.loads(response)     
        ...
        for user in users:
            created = time.strftime('%d/%m/%Y', time.gmtime(user['created']/1000))
            modified = time.strftime('%d/%m/%Y', time.gmtime(user['modified']/1000))
            allPortalUsers.append(user['username'])
            #role = str(user['role'])
            fs.write(user['username'] + "," + str(user['description']) + "," + str(user['access']) + "," + str(created) + "," + str(modified)  + "\n")
            #generate The Content
            userUrl = portalUrl + 'sharing/rest/content/users/' + user['username']
            userParameters = parameters = urllib.urlencode({ 'f' : 'json', 'token': token}) #remember to generate json content
            userResponse = urllib2.urlopen(userUrl, userParameters ).read()
            jsonUserResponse = json.loads(userResponse)
            #get Items
            if string.find(user['username'], 'esri_') == -1:
                items = jsonUserResponse['items']
                print(str(f) + '\t' + user['username'] + '\t' + str(len(items)))
                for item in items:
                    fsi.write(item['owner']+','+item['title']+','+str(item['url'])+','+str(item['numViews'])+','+item['type']+"\n")
...
def deletePortalContent(folder):
    ...
    #todo: admin user list
    adminUserList = ['AmitKokj1@TRANSPORT', 'SusanJon1@TRANSPORT']
...
#output Folder
folder = 'C:\\Users\\Susanjon1\\OneDrive - Auckland Transport\\GIS Azure\\' + envn

#todo: get Current Employees
ef = r"C:\Users\Susanjon1\OneDrive - Auckland Transport\GIS\Current_List_AT_Employees_20-09-2019.csv"
ATEmployees = getATEmployees(ef)
print("Total Auckland Transport employees: " + str(len(ATEmployees)))

#todo: generate Token
print 'generate Token...'
setSecurity (portalUrl + 'sharing/rest/generateToken?', username, password)
print(portalUrl + 'sharing/rest/generateToken?')
token = generateTokenPortal(u, p, portalUrl, 'sharing/rest/generateToken?')
...
#todo: sort Users to get Remove
print("#todo: target Users to Delete")
n = 0
toRemove = open(folder + os.path.sep + "PortalUsersToRemove.csv", "w+")
toRemove.write("UserName\n")
for u in allPortalUsers:
    if u.upper() not in ATEmployees: 
        toRemove.write(u+"\n")
        n += 1
print(str(n) + " users to Delete")
toRemove.close()

#todo: get Role
#print "\ntodo: generate Role"
#getRole = generateRoles(folder)
